// Unit tests for `useSnackbarStack`.
//
...
// deterministic.
//
// `resetSnackbarStackForTesting()` (test-only helper) wipes the store
...
import {
    ...
    useSnackbarStack
} from './snackbar-stack.composable'

describe('useSnackbarStack', () => {
    beforeEach(() => {
        resetSnackbarStackForTesting()
        vi.useFakeTimers()
    })
    ...
    it('notify pushes an item with an auto-generated id and returns it', () => {
        ...
        expect(id).toMatch(/^origam-snackbar-stack-item-/)
    ...
    it('FIFO eviction kicks in past `max`', () => {
        const stack = useSnackbarStack({ max: 2 })
    ...
    it('falls back to `defaultDuration` when duration is undefined', () => {
        const stack = useSnackbarStack({ defaultDuration: 500 })
    ...
    it('keeps separate buckets per `id`', () => {
        const a = useSnackbarStack({ id: 'a' })
        const b = useSnackbarStack({ id: 'b' })
    ...
    it('two callers on the same id share the same store', () => {
        const a = useSnackbarStack({ id: 'shared' })
        const b = useSnackbarStack({ id: 'shared' })
