...
interface ISnackbarStackState {
    items: Ref<Array<ISnackbarStackItem>>
    timers: Map<string, number>
    counter: { current: number }
}

const STORES = new Map<string, ISnackbarStackState>()

const getStore = (id: string): ISnackbarStackState => {
...
const generateId = (state: ISnackbarStackState): string => {
    state.counter.current += 1

    return `origam-snackbar-stack-item-${state.counter.current}`
}

const clearTimer = (state: ISnackbarStackState, itemId: string): void => {
...
export function useSnackbarStack (options: IUseSnackbarStackOptions = {}): IUseSnackbarStackReturn {
    ...
    const notify = (opts: ISnackbarStackItemOptions): string => {
        ...
        // (`<OrigamSnackbarStack max="…">`) enforces its own cap on the
 ...
 * import this.
 ********************************************************/
export function useSnackbarStackInternal (id: string = SNACKBAR_STACK_DEFAULT_ID) {
    const state = getStore(id)
 ...
 ********************************************************/
export function resetSnackbarStackForTesting (): void {
