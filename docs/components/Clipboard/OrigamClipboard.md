...
(`mdi:mdi-content-copy`) that flips its label to the feedback text
...
| `feedbackText`     | `string`  | `'Copied!'` | Label rendered in the default trigger and feedback overlay. Wrap with `t()` for full i18n.     |
| `successText`      | `string`  | `undefined` | Alias for `feedbackText`. Takes precedence when both are passed.                               |
| `showFeedback`     | `boolean` | `false`     | Renders the auto ARIA-live pill in addition to whatever the slot draws.                        |
...
inside the ARIA-live pill when `showFeedback` is true.
...
- When `showFeedback` is true, the overlay carries `role="status"`
  and `aria-live="polite"` so screen readers announce the copy after
  the fact without interrupting the user.
...
- [`OrigamCode`](../Code/OrigamCode.md) — has its own internal copy
