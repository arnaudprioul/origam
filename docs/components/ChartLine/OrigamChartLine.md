...
Line chart with optional smoothing — the canonical time-series visualisation.

Type-specific wrapper around `<OrigamChart type="line">` with a
...
| `smoothing`          | `'none' \| 'curve'`                                | `'none'`      | `'curve'` uses cubic Bezier with Catmull-Rom tangents.                                       |
| `yMin` / `yMax`      | `number`                                          | auto          | Override the auto-computed Y range.                                                           |
| `showGrid`           | `boolean`                                         | `true`        | Render grid lines behind the plot.                                                            |
| `showAxis`           | `boolean`                                         | `true`        | Render X / Y axes + tick labels.                                                              |
| `xAxisFormat`        | `(value) => string`                               | identity      | Formatter applied to X-axis tick labels.                                                      |
| `yAxisFormat`        | `(value) => string`                               | identity      | Formatter applied to Y-axis tick labels.                                                      |
