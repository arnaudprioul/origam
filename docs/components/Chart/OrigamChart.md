...
Universal chart shell that dispatches to one of five rendering families based on the `type` prop. Accepts thirteen visualisation primitives — `line`, `area`, `bar`, `column`, `scatter`, `spline`, `stepped-line`, `pie`, `donut`, `radar`, `gauge`, `funnel`, `pyramid` — through a single consistent API. Pure SVG rendering; no canvas, no `d3`, no `chart.js`.

Prefer a family-level component (`OrigamChartCartesian`, `OrigamChartPolar`, `OrigamChartRadar`, `OrigamChartGauge`, `OrigamChartPyramid`) when the chart type is fixed at compile time and you want a tightly-typed prop surface. Use `OrigamChart` when the type is determined at runtime or you need to switch families dynamically.
...
| Name | Type | Default | Description |
|---|---|---|---|
| `type` | `TChartType` | `'line'` | Visualisation primitive. One of `'line'`, `'area'`, `'bar'`, `'column'`, `'scatter'`, `'spline'`, `'stepped-line'`, `'pie'`, `'donut'`, `'radar'`, `'gauge'`, `'funnel'`, `'pyramid'`. |
