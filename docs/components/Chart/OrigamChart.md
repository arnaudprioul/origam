...
Universal chart shell that dispatches to one of four rendering families based on the `type` prop. Accepts eleven visualisation primitives — `line`, `area`, `bar`, `column`, `scatter`, `spline`, `stepped-line`, `pie`, `donut`, `radar`, `gauge` — through a single consistent API. Pure SVG rendering; no canvas, no `d3`, no `chart.js`.
...
| Name | Type | Default | Description |
|---|---|---|---|
| `type` | `TChartType` | `'line'` | Visualisation primitive. One of `'line'`, `'area'`, `'bar'`, `'column'`, `'scatter'`, `'spline'`, `'stepped-line'`, `'pie'`, `'donut'`, `'radar'`, `'gauge'`. |
