...
Area chart (line + filled region down to the baseline).

Type-specific wrapper around `<OrigamChart type="area">` with a
...
| `smoothing`          | `'none' \| 'curve'`                                | `'none'`      | Path smoothing strategy. `'curve'` for cubic Bezier interpolation.                            |
