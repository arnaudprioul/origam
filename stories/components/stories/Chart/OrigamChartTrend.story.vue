<template>
	<Story
			group="components"
			title="Chart/OrigamChartTrend"
	>
		<Variant
				title="Default"
				:init-state="() => useStoryInitState<IChartTrendProps>({
		...
		>
			<template #default="{ state }">
				<div
						class="story-shell"
						data-cy="chart-trend-default"
				>
					<origam-chart-trend
							v-bind="state"
							data-cy="chart-trend-default-host"
					/>
				</div>
			</template>
		...
		<Variant title="Prop — legendPosition (top / right / bottom / left)">
			<div class="story-shell" data-cy="chart-trend-legend">
				<p class="hint">
				...
				>
					...
					<origam-chart-trend
							...
							:height="220"
							:legend-position="pos"
							:data-cy="`chart-trend-legend-${pos}`"
					/>
				</div>
			</div>
		</Variant>
		...
		<Variant title="Prop — animated (on / off)">
			<div class="story-shell" data-cy="chart-trend-animated">
				<p class="hint">
				...
				<div class="story-col">
					...
					<origam-chart-trend
							...
							:height="240"
							:animated="true"
							data-cy="chart-trend-animated-on"
					/>
				</div>
				<div class="story-col">
					...
					<origam-chart-trend
							...
							:height="240"
							:animated="false"
							data-cy="chart-trend-animated-off"
					/>
				</div>
			</div>
		</Variant>
		...
		<Variant title="Slot — title (custom title block)">
			<div class="story-shell" data-cy="chart-trend-slot-title">
				<p class="hint">
				...
				<origam-chart-trend
						...
						:categories="['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']"
						:height="280"
						data-cy="chart-trend-slot-title-host"
				>
					<template #title>
						<div class="custom-title">
							<span class="custom-title__badge">LIVE</span>
							<strong class="custom-title__text">Trend — custom slot title</strong>
						</div>
					</template>
				</origam-chart-trend>
			</div>
		...
		<Variant title="Slot — empty (no series state)">
			<div class="story-shell" data-cy="chart-trend-slot-empty">
				<p class="hint">
				...
				<origam-chart-trend
						:series="[]"
						:height="200"
						data-cy="chart-trend-slot-empty-host"
				>
					<template #empty>
		...
		<Variant title="Emit — point-click / legend-click / series-toggle (counters)">
			<div class="story-shell" data-cy="chart-trend-emits">
				<p class="hint">
				...
				<origam-chart-trend
						...
						:categories="['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']"
						:height="280"
						data-cy="chart-trend-emits-host"
				...
				<dl class="story-counters">
					<div>
						<dt>point-click</dt>
						<dd data-cy="chart-trend-emit-count-point">{{ counters.pointClick }}</dd>
					</div>
					<div>
						<dt>legend-click</dt>
						<dd data-cy="chart-trend-emit-count-legend">{{ counters.legendClick }}</dd>
					</div>
					<div>
						<dt>series-toggle</dt>
						<dd data-cy="chart-trend-emit-count-toggle">{{ counters.seriesToggle }}</dd>
					</div>
				</dl>
			</div>
		</Variant>
...
>
	...
	import type { IChartTrendProps } from '@origam/interfaces'
