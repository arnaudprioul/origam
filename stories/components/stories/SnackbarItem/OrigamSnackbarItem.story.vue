...
>
  ...
  import type { ISnackbarStackItemAction } from '@origam/interfaces'
  ...
  const sampleActions: ReadonlyArray<ISnackbarStackItemAction> = [
    {
      label: 'Undo',
      intent: 'primary',
      handler: () => {
  ...
  const handleAction = (action: ISnackbarStackItemAction): void => {
    action.handler()
  }
</script>
