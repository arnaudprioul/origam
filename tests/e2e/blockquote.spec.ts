...
test.describe('OrigamBlockquote — Prop lang (decorative quote marks)', () => {
    test('lang=fr renders French guillemets « and »', async ({ page }) => {
        ...
        const openMark = await host.locator('.origam-blockquote__mark--open').textContent()
        const closeMark = await host.locator('.origam-blockquote__mark--close').textContent()
        expect(openMark).toContain('«')
        expect(closeMark).toContain('»')
    })

    test('lang=en renders English curly double quotes “ and ”', async ({ page }) => {
        await openVariant(page, STORY, 'Prop — lang')
        const sandbox = sandboxOf(page)
    ...
    test('lang=de renders German low/high quotes „ and "', async ({ page }) => {
        await openVariant(page, STORY, 'Prop — lang')
        const sandbox = sandboxOf(page)
