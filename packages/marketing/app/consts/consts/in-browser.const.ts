...
export const IN_BROWSER_CONST_DOC: IConstDoc = {
    ...
    // `typeof ${'window'}` keeps the rendered text identical to the DS source
    // while preventing the SSR build's `typeof window` -> "undefined" replace
    // from corrupting these documentation strings (the token is not contiguous
    // in the module source, so @rollup/plugin-replace can't match it).
