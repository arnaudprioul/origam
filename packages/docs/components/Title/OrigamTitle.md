...
interface ITitleProps extends ITagProps, ICommonsComponentProps,
    ...
    IBorderProps {
    text?: string
    family?: TFontFamily // 'sans'|'mono'|'serif'
    size?: TFontSize     // 'xs'|'sm'|'md'|'lg'|'xl'|'2xl'|'3xl'|'4xl'|'5xl'
...
|---|---|---|---|
| `family` | `TFontFamily` | — | Font family token. Sets `--origam-title---font-family` to `var(--origam-font__family---{family})` (sans · mono · serif). When unset, the title keeps its theme font-family. |
| `size` | `TFontSize` | — | Font size token. Sets `--origam-title---font-size` to `var(--origam-font__size---{size})` (xs 0.625rem → 5xl 2.25rem). **Overrides the density-driven font-size** when set; when unset, the density prop continues to control the size via the compact / default / comfortable ramps. |
